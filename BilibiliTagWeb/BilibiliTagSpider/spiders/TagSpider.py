...
class TagSpider(Spider):
    ...
    def __init__(self, type_id=None, time_from=None, time_to=None, *args, **kwargs):
    ...
    def parse_hotlist_init(self, response):
        ...
        if 'numResults' in result and result['numResults'] >= 0:
            ...
            self.logger.error("parse_hotlist_init，热榜分页信息解析失败：{}".format(response.url))
    ...
    def parse_hotlist_one_day(self, response):
        ...
        if 'numResults' in result and result['numResults'] >= 0:
            ...
            self.logger.error("parse_hotlist_one_day，热榜分页信息解析失败：{}".format(response.url))
    ...
    def parse_hotlist_one_day_page(self, response):
        ...
        if 'result' in result and 'numPages' in result:
            ...
            if self.parse_items_total[time_cur]['cur'] == self.parse_items_total[time_cur]['total']:
                ...
                yield item
                self.logger.info(
                    "[分区号{type_id}，{time_cur}]的视频爬取完成".format(
                        type_id = self.TYPE_ID,
                        time_cur = time_cur,
                    )
                )
    ...
    def parse_tag_from_videopage(self, response):
        ...
        在视频主页中解析标签
        """
        tags = response.xpath(r'//body/div[@id="app"]/div[@class="v-wrap"]/div[@class="l-con"]/div[@id="v_tag"]/ul/li[@class="tag"]/a/text()')
